...
# for each line and each index of each line in maze
	if line.find('s') != -1:
	# if s is on that line, line.find will return its index
		sy = y
		# sy is the starting y coordinate now set to the line number of s
		sx = line.find('s')
		# sx is the starting x coordinate now set to the index of s
		break
		# if s was found, stop looking
class Walls:
	def checkWalls:
		for y, line in enumerate(maze):
